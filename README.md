...
If IS_BOUNCING_BALLS is false, the application is a simulation of a solar system, not that different from ours.
...
-- Easy level
Background: The PhysicalObject is to be used in another project. However, the entire Space class must NOT be shared.
Your task is to extract the PhysicalObject to a new project.
Goal: Enable reuse of PhysicalObject

-- Intermediate level
Background: The solar system and bouncing balls applications are to be sold in separate delivery packages.
